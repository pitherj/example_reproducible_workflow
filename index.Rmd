...
<div class="alert alert-warning" style="border: 2px solid #e0a800;">
<strong>⚠️ Notice: changes coming to OSF (September 2026)</strong>

<p>This workflow currently recommends the Open Science Framework (OSF) for project management and archiving. The Center for Open Science has announced that it is discontinuing the OSF Projects workspace — the part of OSF used for day-to-day project collaboration and file storage. Key dates:</p>

<ul>
<li><strong>November 16, 2026</strong> — no new OSF projects or new components can be created.</li>
<li><strong>February 19, 2027</strong> — all existing OSF projects (public and private) become read-only; no further edits or uploads will be possible.</li>
</ul>

<p>Existing public projects, links, and DOIs will keep working and remain viewable/downloadable, but OSF will no longer function as an active collaboration tool. OSF Registries (preregistration) and OSF Preprints are not affected.</p>

<p><strong>This resource will be updated later this fall (2026)</strong> to reflect these changes and to recommend alternative tools where needed. In the meantime, see the <a href="https://www.cos.io/osf-changes">Center for Open Science's OSF Changes page</a> for full details.</p>
</div>
...
The website is meant to be succinct, and used as a form of checklist and minimal guide. Throughout, we provide links to external resources that will help implement the steps of the workflow.  Where available, we link to resources from the University of British Columbia, the authors’ home institution.

We have a new (May 2026) [preprint](https://doi.org/10.32942/X2B37D) introducing a new resource: ["Concise guidelines for producing reliable, reproducible research"](more/2026-05-25_reliable-research-checklist_Pither_Vis-Dunbar.pdf). A complementary R package (in prep) called ["rrchecklist"](https://github.com/pitherj/rrchecklist) is also introduced. Take a look!
...
Pither, J., Vis-Dunbar, M. (2025). An example workflow for a reproducible and discoverable research project. https://pitherj.github.io/example_reproducible_workflow/
...
- (optional) [have ORCID account](https://orcid.org/)
- [have OSF account](https://osf.io/)
- [ensure “R” and RStudio are installed](https://alliance-rdm-gdr.github.io/rdm-jumpstart/6-EXT-RRStudio.html)
...
knitr::include_graphics("more/2026-04-17_workflow_infrastructure.png")
```
...
knitr::include_graphics("more/2026-04-17_directory_structure.png")
```

- while any number of directory structures are suitable, the key feature is ensuring the `data_raw` folder holds only raw data, and ideally is write-protected (read-only) once all data have been acquired.  Once you're sure you've acquired all your raw data, here is how you can achieve this on a Mac / Unix / Linux based computer:

Click on the "terminal" tab in your RStudio console window, and type the following command (assuming your raw data directory name is "data_raw"):

```
chmod a=r data_raw
```

- initialize a `README.md` or `README.txt` file for root- and each sub-directory (can be fleshed out later)
...
- **NOTE** Make sure you ignore all sensitive and copyrighted files, and it’s recommended to ignore data files; for example, you could add “data/data_raw/” to your .gitignore file to ignore everything in that directory.
...
UBC subscribes to [OSF Institutions](https://guides.library.ubc.ca/OSF), which allows UBC students, staff, and faculty to [log in to OSF with their CWL credentials](https://guides.library.ubc.ca/osf). **NOTE**: if you intend on continuing in academia beyond UBC, it's likely best to create your OSF account using your personal email/info, rather than through your UBC CWL (because you may lose access to your CWL credentials).
...
- Use OneDrive or another storage facility to regularly (e.g. weekly) back up your work from your local computer

***

## Zotero

- create a Zotero library (or [shared group library](https://www.zotero.org/groups/) if collaborating) NOTE: store attachments (PDFs) on LOCAL or OneDrive (not on Zotero cloud due to storage limits)
...
You can optionally write your manuscript in R markdown or Quarto (e.g. using the RStudio editor), in which case you can take advantage of version control that you already have set up in your RStudio project.  

If you choose to do this, you can also (optionally) install and use:  
...
- ~~use the ["check-out / check-in" function on OSF](https://help.osf.io/article/280-checkout-files) to manage collaborative editing~~. (this function is no longer available on OSF)

***

## RStudio + Github

- [frequent and regular versioning (commit/push/pull)](https://docs.posit.co/ide/user/ide/guide/tools/version-control.html#integrations) of coding & markdown/Quarto authoring work to GitHub
...
- double-check / update `DATA-DICTIONARY.md` file for both data_raw and outdata directories and ensure `README.md` files in root and data_raw directories up to date and correct.
...
- detail and explain any deviations from pre-registered plans
...
Examples of archives on OSF are provided [here](https://osf.io/jfy35/) and [here](https://osf.io/ujnw2/).  You'll see that these example archives are far from perfect, and do employ different strategies around code/data archiving, but they nonetheless check many of the boxes!
...
- [choose appropriate copyright license](https://ubc-library-rc.github.io/rdm/content/09_License-Research-Data.html). Still unsure? Consult with [subject librarian for literature searching and licencing considerations](https://library.ok.ubc.ca/about/contact/mathew-vis-dunbar/)
...
- include a clear statement illustrating how the archive should be cited; **NOTE**: this archive should have its own unique citation (separate from main paper, and perhaps different author order) 
...
## Option 3: OSF + Borealis Dataverse

For an example of a blended approach, here's a [main archive on OSF](https://osf.io/jfy35/), and for the purpose of ensuring institutional affiliation and memory, a skeleton archive containing only metadata is available on UBC's [Dataverse](https://borealisdata.ca/dataset.xhtml?persistentId=doi:10.5683/SP3/I6BHSX).  
