...
It provides step-by-step pointers on how to ensure a research project is reproducible, discoverable, and appropriately archived. 
...
The website is meant to be succinct, and used as a form of checklist. Each step/point links out to guidelines/resources about that step. Where available, it provides resources from the University of British Columbia, the authors’ home institution.
...
Consult the [happygitwithr.com website](https://happygitwithr.com/connect-intro) for these steps.
...
- set up an appropriate directory structure, as illustrated below
...
- while any number of directory structures are suitable, the key feature is ensuring the `rawdata` folder holds only raw data, and ideally is write-protected once all data have been acquired.  Once you're sure you've acquired all your raw data, here is how you can achieve this on a Mac / Unix / Linux based computer:
...
- initialize a `_README.md` file for root- and each sub-directory (can be fleshed out later)
...
Examples of archives on UBC's Dataverse (TBD).  
